...
pub fn intersect(node1: Node, node2: Node) -> Node {
    ...
    let (id, low, high) = match (node1, node2) {
        ...
        (_, Node::Leaf(false)) => return node2,
        (Node::Leaf(false), _) => return node1,
...
mod tests {
    ...
    #[test]
    fn intersect_returns_tree_that_has_both_combinations() {
        let item1 = Item::new("1");
        let item2 = Item::new("2");

        let universe = Universe::from(vec![item1.clone(), item2.clone()]);
        let tree1 = universe.tree(vec![item1.clone(), item2.clone()]);
