...
pub fn intersect<T: Hash + Eq + Clone + Sync + Send>(forest1: Forest<T>, forest2: Forest<T>) -> Forest<T> {
    if forest1 == forest2 {
        return forest1;
    }
    ...
    match (&forest1, &forest2) {
        ...
        (Forest::Many(matrix), Forest::Unit(set)) if matrix.contains(set) => forest2.clone(),
        (Forest::Unit(set), Forest::Many(matrix)) if matrix.contains(set) => forest1.clone(),

        (Forest::Many(matrix1), Forest::Many(matrix2)) =>
            Forest::many(&matrix1.par_intersection(matrix2).cloned().collect::<Vec<_>>()),
