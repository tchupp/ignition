...
pub fn intersect<T: Hash + Clone + Ord + Sync + Send>(tree1: Tree<T>, tree2: Tree<T>) -> Tree<T> {
    if tree1 == tree2 {
        return tree1;
    }
    ...
    match (&tree1, &tree2) {
        ...
        (Tree::Many(set1), Tree::Many(set2)) =>
            Tree::many(&set1.par_intersection(set2).cloned().collect::<Vec<_>>()),
...
mod tests {
    ...
    #[test]
    fn intersect_returns_common_one() {
    ...
    #[test]
    fn intersect_returns_common_many() {
        let tree1 = Tree::many(&["1", "2", "3"]);
