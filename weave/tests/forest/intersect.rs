...
pub fn forests_are_have_single_commonality<'a, F: Forest<&'a str> + Debug + Eq + Clone>() -> (F, F, F) {
    let forest1 = F::many(&[
        vec!["1", "2"],
        vec!["2", "3"],
    ]);
...
pub fn forests_are_have_multiple_commonality<'a, F: Forest<&'a str> + Debug + Eq + Clone>() -> (F, F, F) {
    let forest1 = F::many(&[
        vec!["1", "2"],
        vec!["2", "3"],
        vec!["3", "4"],
